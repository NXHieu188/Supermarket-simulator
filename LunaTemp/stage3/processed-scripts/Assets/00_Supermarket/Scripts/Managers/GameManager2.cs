...
public class GameManager2 : Singleton<GameManager2>
{
    ...
    public RayCastItem raycastItem;
    ...
    public void UpdateCustomerSize(bool isHor)
    {
        if (isHor)
        {
            ...
            {
                lstCustomer[i].transform.GetChild(0).localScale = new Vector3(3f, 3f, 3f);
            }
        }
        else
        {
            for (int i = 0; i < lstCustomer.Count; i++)
            {
                lstCustomer[i].transform.GetChild(0).localScale = new Vector3(3.8f, 3.8f, 3.8f);
            }
        }
    }
    public void ActiveUICheckOut(TypeCheckOut type)
    {
        ...
        else
        {
            ...
            txtChangeCash.text = "$" + currentCustomer.change;
            currentCustomer.cusAnim.methodPay.SetActive(false);
        }
    ...
    //OtherPlayable
    public void CaculatorMoney(float totalPrice, TypeProduct type)
    {
        ActionGuide(true);
    }
