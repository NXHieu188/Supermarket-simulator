...
public class GameManager1 : Singleton<GameManager1>
{
    ...
    public void GetProductChoose(TypeProduct type)
    {
        productChoose = productData.GetProduct(type);
    }
    public void ActionGuide()
    {
        handGuide.SetActive(false);
    }
    public void CaculatorMoney(float subPrice, TypeProduct type)
    {
        totalMoney -= subPrice;
        ...
        box.transform.DOMoveZ(10.4f, 0.7f).OnComplete(() =>
        {
    ...
    void SpawnProduct(GameObject obj, [Bridge.Ref] Vector3 pos)
    {
        //Debug.Log(obj.name);
